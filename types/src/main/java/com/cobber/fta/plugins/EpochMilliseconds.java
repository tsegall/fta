...
/**
 * Plugin to detect an Epoch time in milliseconds.
 *
 */
public class EpochMilliseconds extends LogicalTypeInfinite {
	private long minObservation = Long.MAX_VALUE;
	private long maxObservation = Long.MIN_VALUE;
	private final static long YEAR_2000_MILLISECONDS = 946_684_800_000L;

	/**
	 * Construct a plugin to detect an Epoch time in milliseconds based on the Plugin Definition.
	 * @param plugin The definition of this plugin.
	 */
	public EpochMilliseconds(final PluginDefinition plugin) {
		super(plugin);
	}
	...
	private boolean validate(final String trimmed) {
		...
		if (trimmed.length() != 13 || !Utils.isNumeric(trimmed))
			return false;

		final long observation = Long.parseLong(trimmed);
		if (observation > System.currentTimeMillis())
			return false;
	...
	public String nextRandom() {
		return String.valueOf(System.currentTimeMillis() - getRandom().nextInt(26 * 60 * 60));
	}
	...
	public PluginAnalysis analyzeSet(final AnalyzerContext context, final long matchCount, final long realSamples, final String currentRegExp,
		...
		if (getHeaderConfidence(context.getStreamName()) < 99 && (cardinality.size() < 10 || minObservation < YEAR_2000_MILLISECONDS || maxObservation > System.currentTimeMillis()))
