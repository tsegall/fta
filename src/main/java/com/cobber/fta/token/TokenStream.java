...
public class TokenStream {
	...
	private Token[] compressedTokens = null;
	private String compressedKey = null;
	 ...
	 */
	public TokenStream(final String trimmed, long occurrences) {
		...
		isNumeric = digits == len;
		isAlpha = alphas == len;
		isAlphaNumeric = digits != 0 && alphas != 0 && digits + alphas == len;
	}
	 ...
	 */
	private boolean mergeable(TokenStream other) {
		if (getKey().length() != other.getKey().length())
			return false;
		for (int i = 0; i < getKey().length() ;i++) {
			char ch1 = getKey().charAt(i);
			char ch2 = other.getKey().charAt(i);
    ...
    public TokenStream simplify() {
		...
		List<Token> updated = new ArrayList<>(Arrays.asList(getCompressedTokens()));
    ...
    private void coalesceCharClasses(List<Token> toFix, final int from, final int to) {
		CharClassToken mergedToken = (CharClassToken)toFix.get(from);
	...
	private void compress() {
        ...
        compressedTokens = newTokens.toArray(new Token[newTokens.size()]);
	...
	private void replaceWithFloat(final ArrayList<Token> newTokens, final int start, final int end, final boolean signed) {
	...
	public Token[] getCompressedTokens() {
		if (!isCompressed)
			compress();
	...
	public boolean matches(String regExp) {
		Automaton automaton = new RegExp(RegExpGenerator.toAutomatonRE(regExp, false), RegExp.AUTOMATON).toAutomaton(new DatatypesAutomatonProvider());
	 ...
	 */
	private boolean matches(State state, int tokenIndex) {
